...
namespace Ocph.DAL.ExpressionHandler
{
    internal class WhereTranslator : ExpressionVisitor
    {
        ...
        protected override Expression VisitMember(MemberExpression m)
        {
            ...
            if (m.Expression != null)
            {
                    ...
                    m.Member.MemberType == MemberTypes.Property)
                {
                    ...
                    sb.Append(entity.TableName).Append(".").Append(p.Name);
                }
        ...
        private bool ParseOrderByExpression(MethodCallExpression expression, string order)
        {
            ...
            if (lambdaExpression.Body as MemberExpression != null)
            {
                if (string.IsNullOrEmpty(_orderBy))
                {
                    _orderBy = string.Format("{0} {1}", (lambdaExpression.Body as MemberExpression).Member.Name, order);
                }
                else
                {
                    _orderBy = string.Format("{0}, {1} {2}", _orderBy, (lambdaExpression.Body as MemberExpression).Member.Name, order);
                }
        ...
        private bool ParseTakeExpression(MethodCallExpression expression)
        {
            ...
            if (int.TryParse(sizeExpression.Value.ToString(), out _))
            {
                return true;
            }
