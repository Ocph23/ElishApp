    ...
    public class ProductService : IProductService
    {
        ...
        public Task<Unit> UpdateUnit(int unitId, Unit unit)
        {
           ...
           if(existsModel==null)
            ...
            var updated = dbContext.Units.Update(x=> new { x.Amount,x.Buy,x.Name,x.Sell},unit, x=>x.Id==unitId);
            if (updated)
                throw new SystemException($"Unit '{unit.Name}' Not Saved !");
        ...
        public Task<Product> Get(int supplierId)
        {
            var result =  dbContext.Products.Where(x=>x.Id==supplierId).FirstOrDefault();
            return Task.FromResult(result);
        }

        public Task<IEnumerable<Product>> Get()
        {
            var results = dbContext.Products.Select().ToList();
